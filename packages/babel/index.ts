...
export function babelReynaTransformPlugin(serverBasePath: string) {
  return function(babel: Babel) {
    const { types: t } = babel;
      ...
      visitor: {
        ImportDeclaration(nodePath: babel.NodePath<babel.types.ImportDeclaration>, state: any) {
          if (/(.*)\.server(\.(js|ts))?/.test(nodePath.node.source.value)) {
            ...
            const serverFilePath = path.relative(serverBasePath, serverFileAbsolutePath);
